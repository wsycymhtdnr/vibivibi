package com.lyf.vibi.ui.recyclerview
...
open class HiRecyclerView @JvmOverloads constructor(
    ...
    fun enableLoadMore(callback: () -> Unit, prefetchSize: Int) {
        if (adapter !is HiAdapter) {
            Log.d("HiRecyclerView", "enableLoadMore must use hiadapter")
            return
        }
    ...
    fun disableLoadMore() {
        if (adapter !is HiAdapter) {
            Log.e("HiRecyclerView", "disableLoadMore must use hiadapter")
            return
        }
    ...
    fun loadFinished(success: Boolean) {
        if (adapter !is HiAdapter) {
            Log.e("HiRecyclerView", "loadFinished must use hiadapter")
            return
        }
